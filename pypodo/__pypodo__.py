...
def list(open=open):
    ...
    if len(sys.argv) > 3:
        sys.exit(
            colored("error : 0 or 1 parameter is needed for pypodo list - the tag", 'red'))
    vide = 'true'
...
def delete(open=open):
    ...
    if len(sys.argv) >= 3:
        ...
        sys.exit(colored(
            "error : 1 or more parameter is needed for pypodo add - indexes to delete in numeric format", "red"))
